<?php
 ...
 class SwifTable implements arrayaccess, iteratoraggregate, countable, serializable
 {
    ...
    const ERROR_COUNT_PK_VALUES = "La pk tiene %1\$n valor(es), pero se especifico %2\$n.";
	 ...
	 * @var String
	/*/
	private static $_conn = ["user" => "SYSDBA", "pass" => "masterkey", "db" => 'path_to/name_db.fdb'];
	...
	/*/
     * Alias magico de SwifTable::table($table).
	 *
	 * @since 1.9.0
    ...
    {
        return "<dl class=\"swiftable_resume\">
                  <dt>DB</dt>
                  <dd>".self::$_conn["db"]."</dd>
                  <dt>Tabla</dt>
                  <dd>".$this->_table."</dd>
	 ...
	 * @return SwifTable
	/*/
    public function __construct($table = false, $virtual = false)
	{
		if(is_null(self::$_instance))
		{
            self::$_instance = ibase_connect(self::$_conn["db"], self::$_conn["user"], self::$_conn["pass"], "UTF8");
			if(ibase_errcode())
				throw new Exception(self::ERROR_CONNECTION);
		}
        if($table)
            $this->table($table, $virtual);
 	...
 	public function table($table)
	{
        ...
        $this->rawQuery('SELECT r.RDB$FIELD_NAME AS "name", r.RDB$DEFAULT_VALUE AS "default_value"
                            FROM RDB$RELATION_FIELDS r
                            WHERE r.RDB$RELATION_NAME = ?
                            ORDER BY r.RDB$FIELD_POSITION;', ['"'.$table.'"'], true);
                            
        foreach($this->result as $row)
            $this->props[trim($row["name"])] = trim($row["default_value"]);
        
        $this->flush();
        
        $this->rawQuery('SELECT s.RDB$FIELD_NAME AS "name"
            FROM RDB$INDEX_SEGMENTS s
            LEFT JOIN RDB$INDICES i ON i.RDB$INDEX_NAME = s.RDB$INDEX_NAME
            LEFT JOIN RDB$RELATION_CONSTRAINTS rc ON rc.RDB$INDEX_NAME = s.RDB$INDEX_NAME
            WHERE       i.RDB$RELATION_NAME = ?
                    AND rc.RDB$CONSTRAINT_TYPE=\'PRIMARY KEY\'
            ORDER BY s.RDB$FIELD_POSITION;', ['"'.$table.'"'], true);

        foreach($this->result as $row)
            $this->_pk[] = trim($row["name"]);
	...
	public function flush()
	{
		...
		if($this->_stmt)
            $this->_stmt = ibase_free_query($this->_stmt);
    ...
    {
        $this->flush();        
        $relations = $this->exec('SELECT s.RDB$FIELD_NAME AS "from",
                                     i2.RDB$RELATION_NAME AS "table",
                                     s2.RDB$FIELD_NAME AS "field"
                                     FROM RDB$INDEX_SEGMENTS s
                                     LEFT JOIN RDB$INDICES i ON i.RDB$INDEX_NAME = s.RDB$INDEX_NAME
                                     LEFT JOIN RDB$RELATION_CONSTRAINTS rc ON rc.RDB$INDEX_NAME = s.RDB$INDEX_NAME
        ...
        foreach($relations as $relation)
            $fkeys[trim($relation["table"])] = ["from" => trim($relation["from"]), "field" => trim($relation["field"])];
	...
	public function exec($query = "", $params = [], $cache = false)
	{
		...
		if(!$this->_stmt)
		{
			if(!($this->_stmt = ibase_prepare(self::$_instance, $query)))
                throw new Exception(sprintf(self::ERROR_PREPARE_STMT, ibase_errcode(), ibase_errmsg()." (SQL = \"$query\")"));
		}
        
        $bind_params = array($this->_stmt);
        if($params)
		{
			foreach($params as &$param)
                $bind_params[] = &$param;
		}
		if(!($registros = call_user_func_array("ibase_execute", $bind_params)))
            throw new Exception(sprintf(self::ERROR_EXECUTE_STMT, ibase_errcode(), ibase_errmsg()));
		else
		{
			$result = [];
			while($registro = ibase_fetch_assoc($registros))
				$result[] = $registro;
		}
	...
	/*/
     * Alias amigables de SwifTable::fnc();
	 *
	 * @since 1.9.0
	 * @param String $field  Campo que sera pasado a la funcion.
	 * @param Bool   $cache  Indica si la consulta sera cacheada.
	...
	/*/
     * Alias amigables de SwifTable::register();
	 *
	 * @since 1.9.0
